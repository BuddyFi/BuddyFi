...
import { saveUserProfile, loadUserProfile, getDefaultProfile, ProfileData } from '@/services/profileService';

export default function ProfilePage() {
  const { publicKey } = useWallet();
  const cardRef = useRef(null);
  const [isEditing, setIsEditing] = useState(false);
  const [profileData, setProfileData] = useState<ProfileData>(getDefaultProfile());
  ...
  const handleEditSubmit = (e: React.FormEvent) => {
    ...
    setEditForm({ ...editForm, skills: skillsText.split(',').map(s => s.trim()).filter(s => s) });
  };
      ...
      <div className="container mx-auto px-4 py-8">
        {!publicKey ? (
          <div className="max-w-lg mx-auto backdrop-blur-xl bg-white/1 border border-white/10 shadow-[0_4px_12px_-2px_rgba(0,0,0,0.3)] rounded-xl p-8 text-center">
            <h2 className="text-xl font-bold mb-4">Connect Wallet to View Your Profile</h2>
            <p className="text-gray-300 mb-6">Connect your wallet to access and edit your profile</p>
            <ConnectWalletButton />
          </div>
        ) : (
          <>
                  ...
                  <div className="mb-8">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {profileData.skills.map((skill: string) => (
                        <Badge key={skill} className="skill-tag bg-purple-300/20 text-white">
                          {skill}
                        </Badge>
                      ))}
                    </div>
                  ...
                  <div className="backdrop-blur-xl bg-white/1 border border-white/10 shadow-[0_4px_12px_-2px_rgba(0,0,0,0.3)] rounded-xl p-6">
                    ...
                    {profileData.githubUsername ? (
                      ...
                      <div className="text-center py-4">
                        <p className="text-sm text-gray-400">Add your GitHub username to see your activity</p>
                      </div>
                    )}
                  </div>
                </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <div className="space-y-2 col-span-1 sm:col-span-2">
                ...
                <Textarea 
                  id="bio"
                  value={editForm.bio}
                  onChange={(e: { target: { value: any; }; }) => setEditForm({ ...editForm, bio: e.target.value })}
                  className="bg-buddyfi-background/50"
                  rows={4}
                />
              </div>
