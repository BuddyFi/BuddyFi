...
const Progress = ({ value, className, indicatorClassName }: { value: number, className?: string, indicatorClassName?: string }) => (
  <div className={`w-full bg-gray-800 rounded-full h-2 ${className || ""}`}>
    <div 
      className={`h-full rounded-full bg-gradient-to-r from-purple-600 to-blue-600 ${indicatorClassName || ""}`} 
      style={{ width: `${value}%` }}
    ></div>
  </div>
);
...
const initialTeamMembers = [
  { 
    ...
    name: "Alex Rivera", 
    role: "Frontend Dev", 
    avatar: "https://images.unsplash.com/photo-1599566150163-29194dcaad36?q=80&w=256", 
...
const TeamDashboard = () => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      setIsLoading(true);
      try {
        const response = await fetch('/api/adminbuddy');
        if (!response.ok) {
        ...
        if (data.users && data.users.length > 0) {
          ...
          const fetchedMembers = data.users.map((user: { userData: { name: any; role: any; profileImage: any; walletAddress: string; }; }, index: number) => ({
            id: (index + 1).toString(),
            ...
            walletAddress: user.userData.walletAddress ? 
              `${user.userData.walletAddress.substring(0, 4)}...${user.userData.walletAddress.substring(user.userData.walletAddress.length - 4)}` : 
              "Unknown"
          }));
  ...
  const hackathonEndDate = new Date("May 25, 2025").getTime();
  ...
  // Rendering helpers
  const getTeamMemberById = (id: string) => teamMembers.find(member => member.id === id);
            ...
            <div className="max-w-6xl mx-auto">
              <div className="backdrop-blur-xl bg-white/2 border border-white/10 shadow-[0_4px_12px_-2px_rgba(0,0,0,0.3)] rounded-xl p-8 text-center">
                ...
                <p className="text-gray-300 mb-6">
                  Connect your wallet to collaborate with your hackathon team
                </p>
  ...
  return (
    <div>
      <Navbar />
      <div className="min-h-screen pt-16 pb-24 md:pb-16 bg-black bg-[radial-gradient(ellipse_at_top_right,rgba(120,0,255,0.15),transparent_50%),radial-gradient(ellipse_at_bottom_left,rgba(0,150,255,0.1),transparent_50%)]">
        <div className="container mx-auto px-4 py-8">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
              <div>
                <h1 className="text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-purple-400 to-blue-500">Team Dashboard</h1>
                <p className="text-gray-400">Collaborate with your hackathon team on Project X</p>
              </div>
              <div className="flex items-center space-x-4">
                <div className="text-right">
                  <div className="flex items-center mb-1">
                    <Clock className="text-purple-400 h-4 w-4 mr-2" />
                    <span className="text-sm text-gray-300">Submission Deadline</span>
                  </div>
                  <div className="text-xl font-mono font-bold text-white">{daysRemaining}d {hoursRemaining}h</div>
                </div>
                <div>
              ...
              <div className="space-y-6">
                ...
                {isLoading ? (
                  ...
                  <div className="divide-y divide-white/5">
                    {teamMembers.map((member) => (
                      <div key={member.id} className="flex items-center p-4 hover:bg-white/5">
                ...
                <Card className="backdrop-blur-xl bg-black/40 border border-white/10 shadow-[0_4px_12px_-2px_rgba(0,0,0,0.3)]">
                  ...
                  <CardContent className="pt-6">
                    ...
                    <Progress value={progressPercentage} className="h-2 bg-gray-800" indicatorClassName="bg-gradient-to-r from-purple-600 to-blue-600" />
                    ...
                    <div className="mt-6 space-y-2">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-sm font-medium">{tasks.filter(t => t.status === "To Do").length}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <div className="flex items-center">
                ...
                <Card className="backdrop-blur-xl bg-black/40 border border-white/10 shadow-[0_4px_12px_-2px_rgba(0,0,0,0.3)]">
                  ...
                  <CardContent className="pt-6">
                    <div className="space-y-4">
                      <a href="#" className="flex items-center p-3 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 transition-colors">
                        <Github className="h-5 w-5 mr-3 text-white" />
                        <div className="flex-1">
                          <div className="font-medium">GitHub Repository</div>
                          <div className="text-sm text-gray-400">github.com/buddyfi/project-x</div>
                        </div>
                      ...
                      <a href="#" className="flex items-center p-3 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 transition-colors">
                        ...
                        <div className="flex-1">
                          <div className="font-medium">Hackathon Guidelines</div>
                          <div className="text-sm text-gray-400">Submission requirements & judging criteria</div>
                        </div>
                        <ArrowRight className="h-4 w-4 text-gray-400" />
                      </a>
                      ...
                      <a href="#" className="flex items-center p-3 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 transition-colors">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-sm text-gray-400">API specs & technical architecture</div>
                        </div>
                        <ArrowRight className="h-4 w-4 text-gray-400" />
                      </a>
                    </div>
                  ...
                  <Tabs defaultValue="all" className="w-full">
                    <div className="border-b border-white/5">
                      <TabsList className="bg-transparent h-12 px-4">
                        <TabsTrigger value="all" className="data-[state=active]:bg-white/10 data-[state=active]:shadow-none rounded-md">
                          All Tasks
                        </TabsTrigger>
                        <TabsTrigger value="mine" className="data-[state=active]:bg-white/10 data-[state=active]:shadow-none rounded-md">
                          My Tasks
                        </TabsTrigger>
                        <TabsTrigger value="upcoming" className="data-[state=active]:bg-white/10 data-[state=active]:shadow-none rounded-md">
                          Upcoming
                        </TabsTrigger>
                      </TabsList>
                    </div>
                              ...
                              <div className="flex items-center justify-between mb-2">
                                <div className="flex items-center">
                                  <div className={`h-2 w-2 rounded-full mr-2 ${
                                    ...
                                    task.status === "Review" ? "bg-orange-500" : "bg-purple-500"
                                  }`}></div>
                                ...
                                <Badge variant="outline" className={`text-xs ${
                                  ...
                                  task.status === "Review" ? "border-orange-500/30 text-orange-400" :
                    ...
                    <TabsContent value="mine" className="m-0">
                      <div className="divide-y divide-white/5">
                        {tasks.filter(task => task.assignedTo === "4").map((task) => {
                          const assignee = getTeamMemberById(task.assignedTo);
                          return (
                            <div key={task.id} className="p-4 hover:bg-white/5">
                              <div className="flex items-center justify-between mb-2">
                        ...
                        {tasks.filter(task => task.assignedTo === "4").length === 0 && (
