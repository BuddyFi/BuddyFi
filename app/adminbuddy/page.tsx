    ...
    <div>
      ...
      <div className="min-h-screen pt-16 pb-24 md:pb-16">
        <div className="container mx-auto px-4 py-8">
          <div ref={cardRef} className="backdrop-blur-xl bg-white/1 border border-white/10 shadow-[0_4px_12px_-2px_rgba(0,0,0,0.3)] rounded-xl p-6">
                      ...
                      >
                        ...
                        <td className="px-4 py-3 font-mono text-sm text-gray-400">
                          {user.userData.walletAddress ? 
                            <>
                              ...
                              {user.userData.walletAddress.substring(user.userData.walletAddress.length - 4)}
                            </> : 
                            "N/A"
                          }
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex flex-wrap gap-1">
                            {Array.isArray(user.userData.skills) && user.userData.skills.length > 0 ? (
                              <>
                                {user.userData.skills.slice(0, 3).map((skill, i) => (
                                  <Badge key={i} className="bg-purple-300/20 text-white text-xs px-2">
                                    {skill}
                                  </Badge>
                                ))}
                        ...
                        <td className="px-4 py-3 text-gray-400">
                          {user.userData.social && user.userData.social.github ? user.userData.social.github : "—"}
                        </td>
                        <td className="px-4 py-3">
                          <Button 
                            variant="outline" 
              ...
              <div className="py-4">
                <div className="flex flex-col md:flex-row gap-6">
                  ...
                  <div className="flex-1">
                    <h2 className="text-2xl font-bold">{selectedUser.userData.name}</h2>
                    <p className="text-gray-400 font-mono text-sm mt-1">
                      {selectedUser.userData.walletAddress || "Wallet address not available"}
                    </p>
                    
                    <div className="mt-4">
                      <h3 className="text-lg font-semibold mb-2">Bio</h3>
                      <p className="text-gray-300 whitespace-pre-line">{selectedUser.userData.bio}</p>
                    </div>
                    
                    <div className="mt-4">
                      <h3 className="text-lg font-semibold mb-2">Skills</h3>
                      <div className="flex flex-wrap gap-2">
                        {Array.isArray(selectedUser.userData.skills) && selectedUser.userData.skills.length > 0 ? (
                          selectedUser.userData.skills.map((skill, i) => (
                            <Badge key={i} className="skill-tag bg-purple-300/20 text-white">
                              {skill}
                            </Badge>
                          ))
                        ) : (
